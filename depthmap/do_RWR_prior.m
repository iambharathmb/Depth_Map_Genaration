...
edgeRegion = ones(N,1);
%% find edge
% im = im2double(img); %An image gradient is a directional change in the intensity or color in an image.For example, the Canny edge detector uses image gradient for edge detection.
% for i = 1:Z 
%     grad(:,:,i)=abs(gradient(im(:,:,i)));  %grad is gradient for single channel
% end
% gradAvg = sum(grad,Z)./Z;                 %take avarage of all channel gradients
% edgeRegion = gradAvg>0.03;
% edgeRegion = imdilate(edgeRegion,ones(2,2));   %dilates the image I, where nhood is a matrix of 0s and 1s that specifies the structuring element neighborhood. imdilate determines the center element of the neighborhood by floor((size(nhood)+1)/2)
